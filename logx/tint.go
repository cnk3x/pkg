...
The output format can be customized using [TintOptions], which is a drop-in
...
// TintOptions for a slog.Handler that writes tinted logs. A zero TintOptions consists
...
func (o *TintOptions) setDefaults() {
	if o.Level == nil {
...
// Tint creates a [slog.Handler] that writes tinted logs to Writer w,
...
// tintHandler implements a [slog.Handler].
...
func (h *tintHandler) appendTintTime(buf *buffer, t time.Time, color int16) {
	if h.opts.NoColor {
...
func (h *tintHandler) appendTintLevel(buf *buffer, level slog.Level, color int16) {
	...
	if !h.opts.NoColor {
		if color >= 0 {
			...
			case level < slog.LevelWarn:
				_, _ = buf.WriteString(ansiBrightGreen)
			case level < slog.LevelError:
				_, _ = buf.WriteString(ansiBrightYellow)
			default:
				_, _ = buf.WriteString(ansiBrightRed)
			}
		}
	}
	...
	switch {
	case level < slog.LevelInfo:
		_, _ = buf.Write(str("DBG", level-slog.LevelDebug))
	case level < slog.LevelWarn:
		_, _ = buf.Write(str("INF", level-slog.LevelInfo))
	case level < slog.LevelError:
		_, _ = buf.Write(str("WRN", level-slog.LevelWarn))
	default:
		_, _ = buf.Write(str("ERR", level-slog.LevelError))
	}
...
func (h *tintHandler) resolve(val Value) (resolvedVal Value, color int16) {
...
func (h *tintHandler) appendAttr(buf *buffer, attr slog.Attr, groupsPrefix string, groups []string) {
	...
	if h.opts.NoColor {
		...
		if color >= 0 {
			appendAnsi(buf, uint8(color), true)
			h.appendKey(buf, attr.Key, groupsPrefix)
			_, _ = buf.WriteString(ansiResetFaint)
			h.appendValue(buf, attr.Value, true)
			_, _ = buf.WriteString(ansiReset)
		} else {
			_, _ = buf.WriteString(ansiFaint)
...
func (h *tintHandler) appendValue(buf *buffer, v Value, quote bool) {
	switch v.Kind() {
...
func (h *tintHandler) appendTintValue(buf *buffer, val Value, quote bool, color int16, faint bool) {
	if h.opts.NoColor {
...
func (v tintValue) LogValue() Value {
	return v.Value
}
